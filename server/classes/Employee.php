...
class Employee {
    ...
    public function all(): array {
        $stmt = $this->conn->query('SELECT e.*, u.name, u.email FROM employees e JOIN users u ON u.id = e.user_id WHERE u.is_active = 1 ORDER BY u.name');
        return $stmt->fetchAll();
    }

    public function find(int $id): ?array {
        $stmt = $this->conn->prepare('SELECT e.*, u.name, u.email FROM employees e JOIN users u ON u.id = e.user_id WHERE e.id = ? LIMIT 1');
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        return $row ?: null;
    }
